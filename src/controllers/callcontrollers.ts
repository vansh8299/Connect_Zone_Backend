...
export const startCall = async (
  _: any,
  { input }: { input: { receiverId: string; sdpOffer: string } },
  context: Context
) => {
  try {
    const currentuserId = getCurrentUserId(context);
    ...
    try {
      // Clean up SDP data to ensure only required fields are passed
      const cleanSdpOffer = input.sdpOffer
        ? JSON.stringify({
            type: JSON.parse(input.sdpOffer).type,
            sdp: JSON.parse(input.sdpOffer).sdp,
          })
        : null;
...
export const answerCall = async (
  _: any,
  { input }: { input: { callId: string; sdpAnswer: string } },
  context: Context
) => {
  try {
    const currentuserId = getCurrentUserId(context);
    ...
    try {
      // Clean up SDP data
      const cleanSdpAnswer = input.sdpAnswer
        ? JSON.stringify({
            type: JSON.parse(input.sdpAnswer).type,
            sdp: JSON.parse(input.sdpAnswer).sdp,
          })
        : null;
...
export const endCall = async (
  ...
  try {
    ...
    // Publish to subscription
    try {
      await pubsub.publish(`callEnded_${input.callId}`, {
        callEnded: endedCall,
      });
...
export const addIceCandidate = async (
  _: any,
  { input }: { input: { callId: string; candidate: string } },
  context: Context
) => {
  try {
    const currentuserId = getCurrentUserId(context);
    console.log(
      "Adding ICE candidate for call:",
      input.callId,
      "by user:",
      currentuserId
    );
...
export const callSubscriptions = {
  callInitiated: {
    ...
    resolve: async (payload: any, _: any, context: Context) => {
      try {
        console.log("callInitiated resolver payload:", payload);

        if (!payload?.callInitiated?.call || !payload.callInitiated.sdpOffer) {
          console.log("Invalid payload structure:", JSON.stringify(payload));
          return null;
        }
        ...
        // Only send to receiver, exclude the caller and verify call status
        if (
          ...
          return {
            call: call,
            sdpOffer: payload.callInitiated.sdpOffer,
            iceCandidate: null,
          };
        }
  ...
  callAnswered: {
    subscribe: async (
      ...
      try {
        ...
        if (
          ...
          throw new GraphQLError("Not authorized to subscribe to this call", {
            extensions: { code: "FORBIDDEN" },
          });
        }

        console.log("User subscribed to callAnswered:", {
          callId,
          userId: currentuserId,
          role: call.callerId === currentuserId ? "caller" : "receiver",
        });
    ...
    resolve: async (
      ...
      try {
        ...
        if (!payload?.callAnswered?.call || !payload.callAnswered.sdpAnswer) {
          console.log(
            "Invalid callAnswered payload structure:",
        ...
        // Only notify caller for their specific call and verify call status
        if (
          payload.callAnswered.call.id === args.callId &&
          call.callerId === currentuserId &&
          call.status === "ONGOING"
        ) {
          console.log("Sending callAnswered to caller:", currentuserId);
          return {
            call: {
              ...payload.callAnswered.call,
              caller: call.caller,
              receiver: call.receiver,
            },
            sdpAnswer: payload.callAnswered.sdpAnswer,
          };
        }
  ...
  callEnded: {
    subscribe: async (
      ...
      try {
        ...
        console.log("User subscribed to callEnded:", {
          callId,
          userId: currentuserId,
          role: call.callerId === currentuserId ? "caller" : "receiver",
        });
    ...
    resolve: async (
      ...
      try {
        ...
        // Check for proper payload structure
        if (!payload?.callEnded?.call) {
          console.log(
            "Invalid callEnded payload structure:",
            JSON.stringify(payload)
          );
          return null;
        }
        ...
        if (
          payload.callEnded?.id === args.callId &&
          (call.callerId === currentuserId || call.receiverId === currentuserId)
        ) {
  ...
  iceCandidateReceived: {
    subscribe: (_: any, { callId }: { callId: string }, context: Context) => {
      try {
        const currentuserId = getCurrentUserId(context);
        console.log(
          "User subscribed to iceCandidateReceived for call:",
          callId,
          "user:",
          currentuserId
        );
    ...
    resolve: async (
      ...
      try {
        ...
        // Check for proper payload structure
        if (
          !payload?.iceCandidateReceived ||
          payload.iceCandidateReceived.callId !== args.callId
        ) {
          console.log(
            "Invalid iceCandidateReceived payload structure or wrong call ID"
          );
          return null;
        }
